# 👋 Hi, I'm Samuel Casanova Valdés

🎯 **Smart Contract Developer | Solidity & Web3 | NFT, Staking, DAO Architectures**  
🔗 CTO @ Distrito Legal | Passionate about building the decentralized future  
🌍 Based in Tenerife, Spain | Open to full-remote roles worldwide  

---

## 🚀 About Me

I'm a smart contract developer with hands-on experience building real-world Web3 systems.  
Currently CTO at a legal-tech startup, with a background in consultancy, product development and technical leadership.

My core strengths:
- Smart contract development with **Solidity**
- Testing using **Foundry**, **Hardhat**, and **Chai/Mocha**
- Designing NFT ecosystems, staking mechanics, DAOs
- Clean, secure code with strong documentation and modular structure

I'm actively looking for **remote senior blockchain developer roles** in international teams.

---

## 🛠️ Tech Stack

- **Languages**: Solidity, JavaScript/TypeScript
- **Web3 Tools**: Hardhat, Foundry, OpenZeppelin, Ethers.js
- **Frontend**: Basic React for dApps
- **Dev Tools**: Git, VSCode, CI/CD, Docker
- **Web3 Infra**: IPFS, Alchemy, Infura, Etherscan APIs

---

## 🌟 Featured Projects

| Project | Description | Tech | Link |
|--------|-------------|------|------|
| 🛒 [NFT Marketplace Advanced](https://github.com/Sulvank/nft-marketplace-advanced) | Multi-collection NFT marketplace with offers, fees and listing logic | Solidity, Hardhat, Ethers.js | [View Repo](https://github.com/Sulvank/nft-marketplace-advanced) |
| 🔄 [Sentinel Swap](https://github.com/Sulvank/sentinel-swap) | Decentralized token swap protocol with liquidity pool management and Uniswap V2-style architecture | Solidity, Foundry | [View Repo](https://github.com/Sulvank/sentinel-swap) |
| 🎟️ [NFT Membership Card](https://github.com/Sulvank/nft-membership-card) | Token-gated NFT membership system with whitelist (Merkle Tree), staking and random draws | Solidity, Foundry, OpenZeppelin | [View Repo](https://github.com/Sulvank/nft-membership-card) |


---

## 📈 Currently

- 🔁 Completing an advanced Web3 training program
- 🧪 Building Solidity projects based on real business cases
- 🎯 Preparing for Web3 technical interviews in English
- 🌍 Open to international remote opportunities

---

## 📫 Contact

- 💼 [LinkedIn](https://www.linkedin.com/in/samuel-casanova-valdes-440b15a6/)
- ✉️ samuelcasanova.int@gmail.com

---

> *“Build useful things. Learn in public. Share the journey.”*
